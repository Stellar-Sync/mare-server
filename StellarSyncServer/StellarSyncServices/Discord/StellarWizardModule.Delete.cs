...
public partial class StellarWizardModule
{
    ...
    public async Task ComponentDelete()
    {
        ...
        await AddUserSelection(stellarDb, cb, "wizard-delete-select").ConfigureAwait(false);
        AddHome(cb);
    ...
    public async Task SelectionDeleteAccount(string uid)
    {
        ...
        using var stellarDb = await GetDbContext().ConfigureAwait(false);
        bool isPrimary = stellarDb.Auth.Single(u => u.UserUID == uid).PrimaryUserUID == null;
        EmbedBuilder eb = new();
    ...
    public async Task ModalDeleteAccountConfirm(string uid, ConfirmDeletionModal modal)
    {
        ...
        try
        {
            ...
            {
                var maxGroupsByUser = _stellarClientConfigurationService.GetValueOrDefault(nameof(ServerConfiguration.MaxGroupUserCount), 3);
