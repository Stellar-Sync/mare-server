...
public partial class StellarWizardModule : InteractionModuleBase
{
    ...
    public async Task ComponentFools()
    {
        ...
        EmbedBuilder eb = new();
        eb.WithTitle("WorryCoin™ and StellarToken© Balance");
    ...
    public async Task ComponentFoolsStart()
    {
        ...
        if (userIsInPermanentVanityRole)
        {
            ...
            eb.WithDescription("Thank you for participating in Stellars 2024 April Fools event."
    ...
    public async Task ComponentFoolsWin(int number)
    {
        ...
        if (hasWon)
        {
            ...
            var user = await Context.Guild.GetUserAsync(Context.User.Id).ConfigureAwait(false);
            await user.AddRoleAsync(_stellarServicesConfiguration.GetValue<ulong?>(nameof(ServicesConfiguration.DiscordRoleAprilFools2024)).Value).ConfigureAwait(false);
        }
        else
        {
