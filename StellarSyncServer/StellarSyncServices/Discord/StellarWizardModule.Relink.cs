...
public partial class StellarWizardModule
{
    ...
    public async Task ComponentRelink()
    {
        ...
        eb.WithDescription("Use this in case you already have a registered Stellar account, but lost access to your previous Discord account." + Environment.NewLine + Environment.NewLine
        ...
        else
        {
            if (verified)
            {
                ...
                eb.WithDescription("This is your private secret key. Do not share this private secret key with anyone. **If you lose it, it is irrevocably lost.**"
                                             ...
                                             + $"||**`{key}`**||"
                                             + Environment.NewLine + Environment.NewLine
                                             + "Enter this key in Stellar Sync and hit save to connect to the service."
                                             + Environment.NewLine + Environment.NewLine
    ...
    private async Task<(string, string)> HandleRelinkUser(StellarDbContext db, string uid)
    {
