...
public partial class StellarWizardModule
{
    ...
    public async Task ComponentUserinfo()
    {
        ...
        await AddUserSelection(stellarDb, cb, "wizard-userinfo-select").ConfigureAwait(false);
        AddHome(cb);
    ...
    public async Task SelectionUserinfo(string uid)
    {
        ...
        await HandleUserInfo(eb, stellarDb, uid).ConfigureAwait(false);
    ...
    private async Task HandleUserInfo(EmbedBuilder eb, StellarDbContext db, string uid)
    {
