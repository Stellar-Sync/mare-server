...
public partial class StellarWizardModule
{
    ...
    public async Task ComponentSecondary()
    {
        ...
        var primaryUID = (await stellarDb.LodeStoneAuth.Include(u => u.User).SingleAsync(u => u.DiscordId == Context.User.Id).ConfigureAwait(false)).User.UID;
        var secondaryUids = await stellarDb.Auth.CountAsync(p => p.PrimaryUserUID == primaryUID).ConfigureAwait(false);
        EmbedBuilder eb = new();
        ...
        eb.WithDescription("You can create secondary UIDs here. " + Environment.NewLine + Environment.NewLine
            + "Secondary UIDs act as completely separate Stellar accounts with their own pair list, joined syncshells, UID and so on." + Environment.NewLine
            + "Use this to create UIDs if you want to use Stellar on two separate game instances at once or keep your alts private." + Environment.NewLine + Environment.NewLine
            + "__Note:__ Creating a Secondary UID is _not_ necessary to use Stellar for alts." + Environment.NewLine + Environment.NewLine
    ...
    public async Task ComponentSecondaryCreate(string primaryUid)
    {
        ...
        ComponentBuilder cb = new();
        AddHome(cb);
        await HandleAddSecondary(stellarDb, eb, primaryUid).ConfigureAwait(false);
        await ModifyInteraction(eb, cb).ConfigureAwait(false);
    }

    public async Task HandleAddSecondary(StellarDbContext db, EmbedBuilder embed, string primaryUID)
    {
        User newUser = new()
        {
            IsAdmin = false,
        ...
        embed.WithDescription("A secondary UID for you was created, use the information below and add the secret key to the Stellar setings in the Service Settings tab.");
