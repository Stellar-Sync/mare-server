...
public class StellarModule : InteractionModuleBase
{
    private readonly ILogger<StellarModule> _logger;
    private readonly IServiceProvider _services;
    private readonly IConfigurationService<ServicesConfiguration> _stellarServicesConfiguration;
    private readonly IConnectionMultiplexer _connectionMultiplexer;

    public StellarModule(ILogger<StellarModule> logger, IServiceProvider services,
        ...
        [Summary("uid", "User ID to the person to send the message to")] string? uid = null)
    {
        ...
        using var db = scope.ServiceProvider.GetService<StellarDbContext>();
        ...
        try
        {
            ...
            await c.PostAsJsonAsync(new Uri(_stellarServicesConfiguration.GetValue<Uri>
            ...
            var discordChannelForMessages = _stellarServicesConfiguration.GetValueOrDefault<ulong?>(nameof(ServicesConfiguration.DiscordChannelForMessages), null);
    ...
    private async Task<EmbedBuilder> HandleUserInfo(EmbedBuilder eb, ulong id, string? secondaryUserUid = null, ulong? optionalUser = null, string? uid = null)
    {
        ...
        using var scope = _services.CreateScope();
        await using var db = scope.ServiceProvider.GetRequiredService<StellarDbContext>();
        ...
        if (primaryUser == null)
        {
            ...
            eb.WithDescription("No Stellar account was found associated to your Discord user");
            return eb;
        }
        ...
        else if ((optionalUser != null || uid != null) && isAdminCall)
        {
            ...
            {
                eb.WithTitle("No account");
                eb.WithDescription("The Discord user has no valid Stellar account");
                return eb;
            }
