...
public partial class StellarWizardModule
{
    ...
    public async Task ComponentRegister()
    {
        ...
        eb.WithDescription("Here you can start the registration process with the Stellar Sync server of this Discord." + Environment.NewLine + Environment.NewLine
        ...
        else
        {
            if (verified)
            {
                ...
                eb.WithDescription("This is your private secret key. Do not share this private secret key with anyone. **If you lose it, it is irrevocably lost.**"
                                             ...
                                             + "**__NOTE: Secret keys are considered legacy. Using the suggested OAuth2 authentication in Stellar, you do not need to use this Secret Key.__**"
                                             + Environment.NewLine + Environment.NewLine
                                             + $"||**`{key}`**||"
                                             + Environment.NewLine + Environment.NewLine
                                             + "If you want to continue using legacy authentication, enter this key in Stellar Sync and hit save to connect to the service."
                                             + Environment.NewLine
    ...
    private async Task<(bool, string)> HandleRegisterModalAsync(EmbedBuilder embed, LodestoneModal arg)
    {
        ...
        embed.WithDescription("Add following key to your character profile at https://na.finalfantasyxiv.com/lodestone/my/setting/profile/"
                              ...
                              + $"**`{lodestoneAuth}`**"
                              + Environment.NewLine + Environment.NewLine
                              + $"**! THIS IS NOT THE KEY YOU HAVE TO ENTER IN STELLAR !**"
                              + Environment.NewLine + Environment.NewLine
                              + "Once added and saved, use the button below to Verify and finish registration and receive a secret key to use for Stellar Sync."
                              + Environment.NewLine
    ...
    private async Task<(string, string)> HandleAddUser(StellarDbContext db)
    {
