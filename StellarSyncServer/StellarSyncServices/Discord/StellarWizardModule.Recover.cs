...
public partial class StellarWizardModule
{
    ...
    public async Task ComponentRecover()
    {
        ...
        eb.WithDescription("In case you have lost your secret key you can recover it here." + Environment.NewLine + Environment.NewLine
            + "## ⚠️ **Once you recover your key, the previously used key will be invalidated. If you use Stellar on multiple devices you will have to update the key everywhere you use it.** ⚠️" + Environment.NewLine + Environment.NewLine
        ...
        await AddUserSelection(stellarDb, cb, "wizard-recover-select").ConfigureAwait(false);
        AddHome(cb);
    ...
    public async Task SelectionRecovery(string uid)
    {
        ...
        await HandleRecovery(stellarDb, eb, uid).ConfigureAwait(false);
        ComponentBuilder cb = new();
        AddHome(cb);
        await ModifyInteraction(eb, cb).ConfigureAwait(false);
    }

    private async Task HandleRecovery(StellarDbContext db, EmbedBuilder embed, string uid)
    {
        ...
        embed.WithDescription("This is your new private secret key. Do not share this private secret key with anyone. **If you lose it, it is irrevocably lost.**"
                              ...
                              + Environment.NewLine + Environment.NewLine
                              + "Enter this key in the Stellar Sync Service Settings and reconnect to the service.");
