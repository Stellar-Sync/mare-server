...
public partial class StellarWizardModule : InteractionModuleBase
{
    private ILogger<StellarModule> _logger;
    private DiscordBotServices _botServices;
    private IConfigurationService<ServerConfiguration> _stellarClientConfigurationService;
    private IConfigurationService<ServicesConfiguration> _stellarServicesConfiguration;
    private IConnectionMultiplexer _connectionMultiplexer;
    private readonly IDbContextFactory<StellarDbContext> _dbContextFactory;
    private Random random = new();

    public StellarWizardModule(ILogger<StellarModule> logger, DiscordBotServices botServices,
        IConfigurationService<ServerConfiguration> stellarClientConfigurationService,
        IConfigurationService<ServicesConfiguration> stellarServicesConfiguration,
        IConnectionMultiplexer connectionMultiplexer, IDbContextFactory<StellarDbContext> dbContextFactory)
    {
        _logger = logger;
        _botServices = botServices;
        _stellarClientConfigurationService = stellarClientConfigurationService;
        _stellarServicesConfiguration = stellarServicesConfiguration;
    ...
    public async Task WizardCaptcha(bool init = false)
    {
        ...
        eb.WithTitle("Stellar Bot Services Captcha");
    ...
    public async Task StartWizard(bool init = false)
    {
        ...
        using var stellarDb = await GetDbContext().ConfigureAwait(false);
        bool hasAccount = await stellarDb.LodeStoneAuth.AnyAsync(u => u.DiscordId == Context.User.Id && u.StartedAt == null).ConfigureAwait(false);

        if (init)
        {
            bool isBanned = await stellarDb.BannedRegistrations.AnyAsync(u => u.DiscordIdOrLodestoneAuth == Context.User.Id.ToString()).ConfigureAwait(false);
...
#if !DEBUG
        bool isInAprilFoolsMode = _stellarServicesConfiguration.GetValueOrDefault<ulong?>(nameof(ServicesConfiguration.DiscordRoleAprilFools2024), null) != null
        ...
        EmbedBuilder eb = new();
        eb.WithTitle("Welcome to the Stellar Sync Service Bot for this server");
        eb.WithDescription("Here is what you can do:" + Environment.NewLine + Environment.NewLine
            ...
            + (hasAccount ? string.Empty : ("- Register a new Stellar Account press \"🌒 Register\"" + Environment.NewLine))
            ...
            + (!hasAccount ? string.Empty : (!isInAprilFoolsMode ? string.Empty : ("- Check your WorryCoin™ and StellarToken© balance and add payment options" + Environment.NewLine)))
        ...
        else
        {
            ...
            {
                cb.WithButton("WorryCoin™ and StellarToken© management", "wizard-fools", ButtonStyle.Primary, new Emoji("💲"));
            }
    ...
    private async Task<StellarDbContext> GetDbContext()
    {
    ...
    private async Task AddUserSelection(StellarDbContext stellarDb, ComponentBuilder cb, string customId)
    {
        var discordId = Context.User.Id;
        var existingAuth = await stellarDb.LodeStoneAuth.Include(u => u.User).SingleOrDefaultAsync(e => e.DiscordId == discordId).ConfigureAwait(false);
        if (existingAuth != null)
        {
            SelectMenuBuilder sb = new();
            sb.WithPlaceholder("Select a UID");
            sb.WithCustomId(customId);
            var existingUids = await stellarDb.Auth.Include(u => u.User).Where(u => u.UserUID == existingAuth.User.UID || u.PrimaryUserUID == existingAuth.User.UID)
    ...
    private async Task AddGroupSelection(StellarDbContext db, ComponentBuilder cb, string customId)
    {
    ...
    private async Task<string> GenerateLodestoneAuth(ulong discordid, string hashedLodestoneId, StellarDbContext dbContext)
    {
