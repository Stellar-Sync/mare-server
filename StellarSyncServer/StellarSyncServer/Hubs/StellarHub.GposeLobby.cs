...
public partial class StellarHub
{
    ...
    {
        _stellarMetrics.IncGauge(MetricsAPI.GaugeGposeLobbyUsers);
        if (priorUsers.Count == 0)
            _stellarMetrics.IncGauge(MetricsAPI.GaugeGposeLobbies);
    ...
    private async Task RemoveUserFromLobby(string lobbyId, List<string> priorUsers)
    {
        ...
        _stellarMetrics.DecGauge(MetricsAPI.GaugeGposeLobbyUsers);

        if (priorUsers.Count == 1)
        {
            await _redis.RemoveAsync($"GposeLobby:{lobbyId}").ConfigureAwait(false);
            _stellarMetrics.DecGauge(MetricsAPI.GaugeGposeLobbies);
        }
        else
        {
