...
public partial class StellarHub : Hub<IStellarHub>, IStellarHub
{
    ...
    private readonly StellarHubLogger _logger;
    ...
    private readonly StellarCensus _stellarCensus;
    ...
    private readonly Lazy<StellarDbContext> _dbContextLazy;
    private StellarDbContext DbContext => _dbContextLazy.Value;
    private readonly int _maxCharaDataByUser;
    private readonly int _maxCharaDataByUserVanity;

    public StellarHub(StellarMetrics stellarMetrics,
        IDbContextFactory<StellarDbContext> stellarDbContextFactory, ILogger<StellarHub> logger, SystemInfoService systemInfoService,
        IConfigurationService<ServerConfiguration> configuration, IHttpContextAccessor contextAccessor,
        IRedisDatabase redisDb, OnlineSyncedPairCacheService onlineSyncedPairCacheService, StellarCensus stellarCensus,
        GPoseLobbyDistributionService gPoseLobbyDistributionService)
    {
        ...
        _stellarCensus = stellarCensus;
        _gPoseLobbyDistributionService = gPoseLobbyDistributionService;
        _logger = new StellarHubLogger(this, logger);
        _dbContextLazy = new Lazy<StellarDbContext>(() => stellarDbContextFactory.CreateDbContext());
    }
    ...
    public async Task<ConnectionDto> GetConnectionDto()
    {
        ...
        _stellarMetrics.IncCounter(MetricsAPI.CounterInitializedConnections);
        ...
        await Clients.Caller.Client_ReceiveServerMessage(MessageSeverity.Information, "Welcome to Stellar Sync \"" + _shardName + "\", Current Online Users: " + _systemInfoService.SystemInfoDto.OnlineUsers).ConfigureAwait(false);
        ...
        return new ConnectionDto(new UserData(dbUser.UID, string.IsNullOrWhiteSpace(dbUser.Alias) ? null : dbUser.Alias))
        {
            ...
            ServerVersion = IStellarHub.ApiVersion,
    ...
    public override async Task OnConnectedAsync()
    {
        ...
        {
            _logger.LogCallWarning(StellarHubLogger.Args(_contextAccessor.GetIpAddress(), "UpdatingId", oldId, Context.ConnectionId));
            _userConnections[UserUID] = Context.ConnectionId;
        }
        else
        {
            _stellarMetrics.IncGaugeWithLabels(MetricsAPI.GaugeConnections, labels: Continent);

            try
            {
                _logger.LogCallInfo(StellarHubLogger.Args(_contextAccessor.GetIpAddress(), Context.ConnectionId, UserCharaIdent));
    ...
    public override async Task OnDisconnectedAsync(Exception exception)
    {
        ...
        {
            _stellarMetrics.DecGaugeWithLabels(MetricsAPI.GaugeConnections, labels: Continent);
            ...
            try
            {
                ...
                _logger.LogCallInfo(StellarHubLogger.Args(_contextAccessor.GetIpAddress(), Context.ConnectionId, UserCharaIdent));
                if (exception != null)
                    _logger.LogCallWarning(StellarHubLogger.Args(_contextAccessor.GetIpAddress(), Context.ConnectionId, exception.Message, exception.StackTrace));

                await RemoveUserFromRedis().ConfigureAwait(false);

                _stellarCensus.ClearStatistics(UserUID);
        ...
        {
            _logger.LogCallWarning(StellarHubLogger.Args(_contextAccessor.GetIpAddress(), "ObsoleteId", UserUID, Context.ConnectionId));
        }
