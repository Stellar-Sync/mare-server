...
{
    public string UserCharaIdent => Context.User?.Claims?.SingleOrDefault(c => string.Equals(c.Type, StellarClaimTypes.CharaIdent, StringComparison.Ordinal))?.Value ?? throw new Exception("No Chara Ident in Claims");

    public string UserUID => Context.User?.Claims?.SingleOrDefault(c => string.Equals(c.Type, StellarClaimTypes.Uid, StringComparison.Ordinal))?.Value ?? throw new Exception("No UID in Claims");

    public string Continent => Context.User?.Claims?.SingleOrDefault(c => string.Equals(c.Type, StellarClaimTypes.Continent, StringComparison.Ordinal))?.Value ?? "UNK";
    ...
    private async Task DeleteUser(User user)
    {
        ...
        _stellarMetrics.IncCounter(MetricsAPI.CounterUsersRegisteredDeleted, 1);
    ...
    {
        _logger.LogCallInfo(StellarHubLogger.Args(dto));
        ...
        if (ownerHasLeft)
        {
            ...
            {
                _logger.LogCallInfo(StellarHubLogger.Args(dto, "Deleted"));
            ...
            else
            {
                ...
                {
                    _logger.LogCallInfo(StellarHubLogger.Args(dto, "Migrated", groupHasMigrated.Item2));
        ...
        _logger.LogCallInfo(StellarHubLogger.Args(dto, "Success"));
