...
public partial class StellarHub
{
    ...
    public async Task<List<OnlineUserIdentDto>> UserGetOnlinePairs(CensusDataDto? censusData)
    {
        ...
        _stellarCensus.PublishStatistics(UserUID, censusData);
    ...
    {
        _logger.LogCallInfo(StellarHubLogger.Args(user));
    ...
    {
        _logger.LogCallInfo(StellarHubLogger.Args(dto.CharaData.FileReplacements.Count));
        ...
        foreach (var replacement in dto.CharaData.FileReplacements.SelectMany(p => p.Value))
        {
            ...
            {
                _logger.LogCallWarning(StellarHubLogger.Args("Invalid Data", "GamePaths", validGamePaths, string.Join(",", invalidPaths), "Hash", validHash, replacement.Hash, "FileSwap", validFileSwapPath, replacement.FileSwapPath));
                hadInvalidData = true;
        ...
        _logger.LogCallInfo(StellarHubLogger.Args(recipientUids.Count));
        ...
        _stellarCensus.PublishStatistics(UserUID, dto.CensusDataDto);

        _stellarMetrics.IncCounter(MetricsAPI.CounterUserPushData);
        _stellarMetrics.IncCounter(MetricsAPI.CounterUserPushDataTo, recipientUids.Count);
    }
