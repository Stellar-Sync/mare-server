...
public class StellarConfigurationServiceClient<T> : IHostedService, IConfigurationService<T> where T : class, IStellarConfiguration
{
    ...
    private readonly ILogger<StellarConfigurationServiceClient<T>> _logger;
    ...
    private Uri GetRoute(string key, string value)
    {
        if (_config.CurrentValue.GetType() == typeof(ServerConfiguration))
            return new Uri((_config.CurrentValue as ServerConfiguration).MainServerAddress, $"configuration/StellarServerConfiguration/{nameof(StellarServerConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(StellarConfigurationBase))
            return new Uri((_config.CurrentValue as StellarConfigurationBase).MainServerAddress, $"configuration/StellarBaseConfiguration/{nameof(StellarBaseConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(ServicesConfiguration))
            return new Uri((_config.CurrentValue as ServicesConfiguration).MainServerAddress, $"configuration/StellarServicesConfiguration/{nameof(StellarServicesConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(StaticFilesServerConfiguration))
            return new Uri((_config.CurrentValue as StaticFilesServerConfiguration).MainFileServerAddress, $"configuration/StellarStaticFilesServerConfiguration/{nameof(StellarStaticFilesServerConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");

        throw new NotSupportedException("Config is not supported to be gotten remotely");
    }

    public StellarConfigurationServiceClient(ILogger<StellarConfigurationServiceClient<T>> logger, IOptionsMonitor<T> config, ServerTokenGenerator serverTokenGenerator)
    {
        _config = config;
        _logger = logger;
    ...
    {
        _logger.LogInformation("Starting StellarConfigurationServiceClient");
